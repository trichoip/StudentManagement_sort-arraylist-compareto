...
// STUDENT REPORT
    ...
    public void showInfoSubjectSortNameByStudentId(String id) {
        ...
        Collections.sort(gl, new Comparator<Grade>() {
            @Override
            public int compare(Grade o1, Grade o2) {
                String a = sbl.get(o1.getSubjectID()).getSubjectName();
                String b = sbl.get(o2.getSubjectID()).getSubjectName();
        ...
        for (Grade gra : gl) {
            if (gra.getStudentID().equalsIgnoreCase(id) == true) {
                String nameSort = sbl.get(gra.getSubjectID()).getSubjectName();
...
Collections.sort(gl, new Comparator<Grade>() {
    @Override
    public int compare(Grade o1, Grade o2) {
        String a = stl.get(o1.getStudentID()).getFirstName();
        String b = stl.get(o2.getStudentID()).getFirstName();
        return a.compareTo(b);
    }
});
        for (Grade gra : gl) {
            if (gra.getSubjectID().equalsIgnoreCase(id) == true) {
                String nameSort = stl.get(gra.getStudentID()).getFirstName() + " " + stl.get(gra.getStudentID()).getLastName();
                double average = gl.averageMark(gra.getLabsPoint(), gra.getProgressTestsPoint(), gra.getFinalExamPoint());
                String checkPass = gl.checkPass(gra.getLabsPoint(), gra.getProgressTestsPoint(), gra.getFinalExamPoint());
